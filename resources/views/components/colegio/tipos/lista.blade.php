@php
    // Contamos directamente la colección que nos llega
        ...
        <div class="relative overflow-y-auto scrollbar-thin p-4 {{ $sm_clase }} {{ $md_clase }}">
            ...
            @foreach ($seccion->publicaciones as $publicacion)
                <div x-data="{ openDetail: false }" @click="openDetail = true"
                    class="{{ $publicacion->css_estado }} relative group shadow-lg mb-6 flex flex-row items-center cursor-pointer transition-all overflow-hidden w-full snap-start border border-gray-100 bg-white rounded-lg">

                    @if ($canManagePost)
                        <x-colegio.menukebab :id="$publicacion->id" :prioridad="$publicacion->prioridad" :max="$seccion->publicaciones->max('prioridad')"
                            class="absolute top-2 right-2 z-10 admin-only" />
                    @endif

                    @if ($publicacion->imagen)
                        <div class="flex-shrink-0 w-1/3 sm:w-1/4 md:w-1/5 h-32 sm:h-56 md:h-64 bg-gray-200">
                    ...
                    <div class="p-4 flex flex-col justify-between w-2/3 sm:w-3/4 md:w-4/5">
                        <div>
                            ...
                            <h3
                                class="font-bold text-xs md:text-xl transition-all {{ !is_null($publicacion->detalles) ? 'group-hover:text-green-800' : '' }}">
                                {{ $publicacion->titulo }}
                            </h3>

                            @if ($publicacion->fecha_inicio)
                                <p class="text-xs sm:text-base mt-1 text-gray-500">
                                    {{ $publicacion->fecha_inicio->translatedFormat('d \d\e M \d\e Y') }}
                                </p>
                            @endif
                        </div>

                        <div class="hidden md:flex mt-2 text-sm sm:text-base text-gray-600">
