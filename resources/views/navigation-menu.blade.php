        ...
        <div class="flex justify-between h-16">
            ...
            <div class="hidden lg:flex lg:items-center lg:space-x-1">
                ...
                @auth
                    ...
                    {{-- MENÚ ADMINISTRATIVO --}}
                    @if ($user->tieneAccesoGlobalPagos() || $user->isUsuarioColegio() || $user->can('manage-everything'))
                        ...
                        <div class="space-y-1">
                            @can('manage-everything')
                                <x-responsive-nav-link href="{{ route('admin.users') }}" :active="request()->routeIs('admin.users')"
                                    class="rounded-lg">Usuarios</x-responsive-nav-link>
                            @endcan

                            @if ($user->tieneAccesoGlobalPagos() || $user->isUsuarioColegio())
                                <x-responsive-nav-link href="{{ route('admin.pagos') }}" :active="request()->routeIs('admin.pagos')"
                                    class="rounded-lg">Caja / Cobros</x-responsive-nav-link>
                                <x-responsive-nav-link href="{{ route('admin.socios') }}" :active="request()->routeIs('admin.socios')"
                                    class="rounded-lg">Socios</x-responsive-nav-link>
                                <x-responsive-nav-link href="{{ route('admin.tarifas') }}" :active="request()->routeIs('admin.validar')"
                                    class="rounded-lg text-emerald-600 font-medium bg-emerald-50/50">
                                    Tarifas</x-responsive-nav-link>
                                <x-responsive-nav-link href="{{ route('admin.historial') }}" :active="request()->routeIs('admin.validar')"
                                    class="rounded-lg text-emerald-600 font-medium bg-emerald-50/50">
                                    Historial</x-responsive-nav-link>
                                <x-responsive-nav-link href="{{ route('admin.cuentas') }}" :active="request()->routeIs('admin.validar')"
                                    class="rounded-lg text-emerald-600 font-medium bg-emerald-50/50">
                                    Cuentas Bancarias</x-responsive-nav-link>
                                <x-responsive-nav-link href="{{ route('admin.validar') }}" :active="request()->routeIs('admin.validar')"
                                    class="rounded-lg flex text-emerald-600 font-medium bg-emerald-50/50"><x-ri-check-double-line
                                        class="w-5 h-5 mr-3 text-slate-400 group-hover:text-emerald-500" />
                                    Validar Pagos</x-responsive-nav-link>
                            @endif
                        </div>
                    </div>
                @endif
                ...
                <div class="border-t border-slate-100 mt-6 pt-4 space-y-1">
                    ...
                    <form method="POST" action="{{ route('logout') }}" x-data>
                        ...
                        <x-responsive-nav-link href="{{ route('logout') }}" @click.prevent="$root.submit();"
                            class="text-red-600 hover:bg-red-50 rounded-lg">Cerrar Sesión</x-responsive-nav-link>
                    </form>
                </div>
            @else
                <div class="border-t border-slate-100 mt-4 pt-4 px-2">
                    <a href="{{ route('login') }}"
                        class="flex w-full justify-center items-center px-4 py-3 bg-slate-900 text-white font-bold rounded-xl shadow-lg hover:bg-slate-800 transition transform active:scale-95">Ingresar
                        al Sistema</a>
                </div>
            @endauth
        </div>
